...
public class OfertaDAOImpl implements OfertaDAO {
	...
	static {
		...
		of1.setFecaInicio(LocalDate.of(2017, 5, 1));
		ofertas.add(of1);

		Oferta of2 = new Oferta();
		of2.setCodigoOferta(2);
		of2.setReferencia("FIB330");
		of2.setFecaInicio(LocalDate.of(2017, 5, 16));
		ofertas.add(of2);
	...
	@Override
	public LocalDate getFechaOferta(int codigoOferta) {
		for (Oferta oferta : ofertas) {
			if (oferta.getCodigoOferta() == codigoOferta) {
				return oferta.getFecaInicio();
			}
		}
		throw new IllegalArgumentException();
	}
